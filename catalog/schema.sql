-- The TMDB tables that Watchkeep reads. They mirror the `benito` database of
-- Benito TV, which fills them (a pg_dump of its schema, with the owner and the
-- privileges removed). Watchkeep only reads them. This file creates the same
-- tables for the tests and for a setup without Benito.
