...
| `backend/crates/watchkeep/` | The server and the CLI. `config.rs` (clap `Cli`, `Config`, the `env` and `defaults` constants), `app.rs` (`AppContext`, router, static files), `http/api/` (one file per resource, typed query and response structs in `params.rs` and `responses.rs`), `http/webhook/` (`plex.rs`, `scrobble.rs`), `scrobble/` (the `Scrobbler` and `event.rs`, the body of the generic webhook), `views.rs`, `actions.rs`, `plex/`, `trakt/`. |
