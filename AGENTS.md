...
- `POST /webhook/scrobble` is the generic scrobble endpoint. `event_id` goes into `plays.external_id` with the source `scrobble`, so a second delivery adds no play. An event that is older than `progress.updated_at` leaves the position alone.
