...
- A service function that is one unit of work gets `#[instrument(skip_all, err)]`, never `skip(self)`: the arguments of a service carry search text, titles, and other words of a person, and a span must not export them. Name the fields that help (`fields(target.kind = ..., target.id = %id)`) one by one. A span that crosses a boundary also gets an `otel.kind`: `server` for a request and `consumer` for one unit of timed work. A field that the function learns later starts as `field::Empty` and gets `Span::record`.
