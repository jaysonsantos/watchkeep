...
- `POST /webhook/scrobble` is the generic scrobble endpoint. `event_id` goes into `plays.external_id` with the source `scrobble`, so a second delivery adds no play. An event that is older than `progress.updated_at` leaves the position alone, and a play that lands inside the rewatch window of a play that the item already has adds nothing. The rewatch window of this endpoint looks both ways (`play_near`), because senders deliver events out of order. An `unwatched` event follows the same rule, and a position event answers `already-watched` when a play of the rewatch window already finished the item, the same as the Plex path. The two position writes of this path (`set_progress_if_newer`, `clear_progress_if_older`) carry the comparison in the statement, because two requests for one item can overlap. An `unwatched` event that is older than the newest play leaves that play alone. A `tmdb` id that `tmdb_number` discards is no identity, and a negative `position_ms` is no position: both give an error, because a blank row could never match again. The handler holds the item with `lock_target` for its transaction, so the checks of one event and its writes cannot interleave with another event of the same item. An episode needs the ids or the title of its show: the catalog is optional, so the ids of the episode alone cannot name the show.
